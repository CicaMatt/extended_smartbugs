...
contract PeoplesATM is Governance, ERC20Detailed{

    using SafeMath for uint256;

    //events
    event eveSetRate(uint256 burn_rate, uint256 reward_fee);
    ...
    // hardcode limit rate
    uint256 public constant _maxGovernValueRate = 1200; //1200
    uint256 public constant _minGovernValueRate = 0;  //0/500
    uint256 public constant _rateBase = 100;

   
    // additional variables for use
    uint256 public  _burnRate = 10;
    uint256 public  _rewardFee = 10;

    uint256 public _totalBurnToken = 0;
    uint256 public _totalRewardToken = 0;

    address public _rewardPool = 0x14723A09ACff6D2A60DcdF7aA4AFf308FDDC160C;
    ...
    */
    constructor (address governance) public ERC20Detailed("PeoplesATM", "PATM", 18) {
        _totalSupply = 1200 * (10**18); // 12000
        _minimumSupply = _totalSupply.sub(700 * (10**18));
    ...
    */
    ...
    function minimumSupply() external view
    returns (uint256) {
    ...
    */
    function setRate(uint256 burn_rate, uint256 reward_fee) public
        onlyGovernance
    {
        require(_maxGovernValueRate >= burn_rate && burn_rate >= _minGovernValueRate,"invalid burn rate");
        require(_maxGovernValueRate >= reward_fee && reward_fee >= _minGovernValueRate,"invalid reward rate");

        _burnRate = burn_rate;
        _rewardFee = reward_fee;

        emit eveSetRate(burn_rate, reward_fee);
    }
    ...
    */
    ...
    returns (bool)
    {
        ...
        require(_totalSupply > _minimumSupply);
        ...
        uint256 rewardTax = (value.mul(_rewardFee)).div(_rateBase);
        if (rewardTax > 0) {
           //to reward
            _balances[_rewardPool] = _balances[_rewardPool].add(rewardTax);
            sendAmount = sendAmount.sub(rewardTax);

            _totalRewardToken = _totalRewardToken.add(rewardTax);

            emit Transfer(from, _rewardPool, rewardTax);
        }
