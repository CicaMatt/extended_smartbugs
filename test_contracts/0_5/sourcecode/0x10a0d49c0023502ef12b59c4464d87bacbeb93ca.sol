...
contract ERC20 is IERC20 {
    ...
    function maxPayoutOf(uint256 _amount) pure external returns(uint256) {
        return _amount*200/100; // 200% max Payout
    }
    ...
    function payoutOf(address _customerAddress) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(totalPayout[_customerAddress] < max_payout) {
            payout = (tokenStakeBalanceLedger_[_customerAddress] * 3 * ((block.timestamp - stakeTime[_customerAddress]) / 1 minutes) / 100) - totalPayout[_customerAddress];
    ...
    function withdrawMonthlyStakeInternal() internal {
        ...
        require(Token(tokenAddr).balanceOf(address(this))>to_payout, "Less Token Balance in Contract");
        require(Token(tokenAddr).transfer(_customerAddress, to_payout),"Couldn't Transfer");
...
contract CapitalSwissCoin is ERC20Detailed,ERC20,Ownable {


    constructor(address _tokenAddr)
        public
         ERC20Detailed ("Capital Swiss Coin", "CSC", 18) {
